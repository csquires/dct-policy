...
def plot_results_vary_density(
    ...
    random_results = np.zeros([len(density_list), ngraphs])
    dct_results = np.zeros([len(density_list), ngraphs])
    opt_single_results = np.zeros([len(density_list), ngraphs])
    coloring_results = np.zeros([len(density_list), ngraphs])
    greedy_minmax_results = np.zeros([len(density_list), ngraphs])
    greedy_entropy_results = np.zeros([len(density_list), ngraphs])

    verification_optimal = np.zeros([len(density_list), ngraphs])
    clique_sizes = np.zeros([len(density_list), ngraphs])
    num_cliques = np.zeros([len(density_list), ngraphs])
    dags_array = np.zeros([len(density_list), ngraphs], dtype=object)
    num_arcs = np.zeros([len(density_list), ngraphs])
    ...
    plt.clf()
    if 'random' in algorithms:
        plt.plot(density_list, (random_results/verification_optimal).mean(axis=1), color=POLICY2COLOR['random'], label='ND-Random')
    if 'dct' in algorithms:
        plt.plot(density_list, (dct_results/verification_optimal).mean(axis=1), color=POLICY2COLOR['dct'], label='DCT')
    if 'opt_single' in algorithms:
        plt.plot(density_list, (opt_single_results/verification_optimal).mean(axis=1), color=POLICY2COLOR['opt_single'], label='OptSingle')
    if 'coloring' in algorithms:
        plt.plot(density_list, (coloring_results/verification_optimal).mean(axis=1), color=POLICY2COLOR['coloring'], label='Coloring')
    if 'greedy_minmax' in algorithms:
        plt.plot(density_list, (greedy_minmax_results/verification_optimal).mean(axis=1), color=POLICY2COLOR['greedy_minmax'], label='Greedy Minmax')
    if 'greedy_entropy' in algorithms:
    ...
    plt.clf()
    if 'random' in algorithms:
        plt.plot(density_list, (random_results/verification_optimal).max(axis=1), color=POLICY2COLOR['random'], label='ND-Random')
    if 'dct' in algorithms:
        plt.plot(density_list, (dct_results/verification_optimal).max(axis=1), color=POLICY2COLOR['dct'], label='DCT')
    if 'opt_single' in algorithms:
        plt.plot(density_list, (opt_single_results/verification_optimal).max(axis=1), color=POLICY2COLOR['opt_single'], label='OptSingle')
    if 'coloring' in algorithms:
        plt.plot(density_list, (coloring_results/verification_optimal).max(axis=1), color=POLICY2COLOR['coloring'], label='Coloring')
    if 'greedy_minmax' in algorithms:
